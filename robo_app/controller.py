...
class CreateBatchController():
    ...
    def load_bookmarks_data(self):
        ...
        user_bookmark_file = os.path.join(AppConfig.user_bookmarks_path,os.path.basename(project_path),'bookmarks.json')
        self.bookmark_dd= {}
    ...
    def callback_create_batch(self):
        if len(self.createbatch_view.get_batch_tests()) > 0:
            ...
            messagebox.showinfo("Create Batch Error", "Are you sure you want to create empty batch. I don't think so. Please add some scripts.", parent=self.createbatch_view)
    ...
    def callback_create_bookmark(self):
        ...
        if len(test_list) > 0:
            ...
            messagebox.showinfo("Create Batch Error", "Are you sure you want to create empty bookmark. I don't think so. Please add some scripts.", parent=self.createbatch_view)
    ...
    def cmd_insert_batch_details(self):
        ...
        if len(error_list) == 0:
            ...
            browser_device_list =self.createbatch_view.inputs['lstbx_device'].get_selected_values().replace("\n",';') if self.createbatch_view.inputs['rb_applicationTypeWeb'].variable.get() == 'Mobile' else self.createbatch_view.inputs['lstbx_browser'].get_selected_values().replace("\n",';')
            ...
            batch_id = self.batch_model.cmd_insert_batch_details(batch_name, test_type, browser_device_list, thread_count,result_location,project_location )
            if batch_id:
                ...
                if self.createbatch_view.inputs['rb_applicationTypeWeb'].variable.get() == 'Mobile':
                    ...
                    self.batch_model.cmd_insert_command_variable(batch_id,alm_test_plan_path=alm_test_plan,
                                                                 ...
                                                                 alm_proj=RunTimeData().getdata('alm_project'),
                                                                 env_url=env_url)
                messagebox.showinfo('Batch Create', "Batch has been created with Batch ID:{}".format(batch_id), parent=self.createbatch_view)
            else:
                messagebox.showerror('Error', 'Unable to Create Batch')
            self.createbatch_view.inputs['win_batchdetails'].destroy()
        else:
            messagebox.showerror('Error', 'Hold On. Understand you are in Hurry but I think you forgot something', parent=self.createbatch_view)


    def get_device_list(self):
        patrn = "[\dA-Za-z-]+"
        file_loc =  os.path.join(self.createbatch_view.inputs['txb_ProjectLocation'].variable.get(),r'Resources/ConfigFiles/DeviceCapList.py')
...
class BatchMonitorController():
    ...
    def callback_open_selected(self):
        ...
        self.batch_execution_monitor_controller = BatchExecutionMonitorController(self.batch_monitor_view, batch_id=row[0].Batch_ID)
    ...
    def callback_tree_update(self):
        ...
        self.batch_update_controller = BatchUpdateController(self.batch_monitor_view,batch_id=row.Batch_ID,callbacks=callback)
    ...
    def callback_tree_open(self):
        ...
        self.batch_execution_monitor_controller = BatchExecutionMonitorController(self.batch_monitor_view, batch_id=row.Batch_ID)
    ...
    def callback_tree_stop(self):
        ...
        # print(self.execution_threads.keys())
...
class BatchExecutionMonitorController():
    ...
    def __init__(self, parent,batch_id, *args, **kwargs):
        ...
        callbacks = {'Open':self.callback_tree_open,
                     ...
                     'Stop':self.callback_tree_stop}
        self.batch_exec_monitor_view =  v.BatchExecutionMonitor(parent, callbacks, batch_id=self.batch_id,  *args, **kwargs)
    ...
    def callback_update_batch_details(self):
         try:
            ...
            if len(error_list)> 0:
                ...
                device_browser_list = data['lstbx_device'] if data['rb_applicationTypeWeb']=='Mobile' else data['lstbx_browser']
                bol_batch_update= self.batch_update_model.cmd_update_batch_details(self.batch_id, data['rb_applicationTypeWeb'],
                                                                device_browser_list,
                                                                data['txb_batchNumberOfThreads'])
                bol_var_update = self.batch_update_model.cmd_update_command_variable(self.batch_id, data['txb_alm_plan_path'],
                                                                   data['txb_alm_lab_path'], data['txb_alm_test_set_name'],
                ...
                bol_script_update = self.batch_update_model.cmd_update_scripts(self.batch_id, device_browser_list.split(';'))
            ...
            try:
                if self.create_bookmark_model.exiting_bookmark(bm_name) :
                    if (messagebox.askokcancel(title="Existing BookMark Update",
                                           message="You are trying to update existing Bookmarks.Are you sure?", parent=self.create_bookmark_view)):
                        pass_word = self.create_bookmark_view.load_admin_ui()
                        if pass_word == 'Pr1y@':
                            if self.create_bookmark_model.add_project_book_mark(data_dict):
                                messagebox.showinfo("Sucess!!", "Bookmark has been created/updated.", parent=self.create_bookmark_view)
                    ...
                    if self.create_bookmark_model.add_user_book_mark(data_dict):
                        messagebox.showinfo("Success!!", "Bookmark has been created/updated.", parent=self.create_bookmark_view)
                        self.create_bookmark_view.destroy()
            except Exception as e:
                messagebox.showerror('Error', 'Unable to Create BookMark. Please Contact Dev Team', parent=self.create_bookmark_view)

        else:
            messagebox.showerror('Error', 'Hold On. Understand you are in Hurry but I think you forgot something', parent=self.create_bookmark_view)
        ...
        if len(error_list) == 0:
            ...
            try:
                if self.login_model.authenticate(view_data['txb_user_name'],
                                                 view_data['txb_user_pass']) and view_data['cb_domain'] !='' and view_data['cb_project'] !='' :
